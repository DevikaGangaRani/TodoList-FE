...
const TodoComponent = () => {
  ...
  const showAll = (page: number = 1) => {
    setLoading(true);
    axios
      .post("http://localhost:5566/todo/readAll", { page })
      .then((response) => {
  ...
  const handleOk = (formData: any) => {
    const reqBody = {
      ...
      DateToComplete: formData.DateToComplete,
    };
    if (!currentItem) {
      axios
      ...
      axios
        .post(`http://localhost:5566/todo/update/${formData.id}`, formData)
        .then(() => {
          setVisible(false);
          setCurrentItem(null);
          showAll();
  ...
  const limitShowAll = () => {
    if (Status && StartDate && CompletedDate && StartDate < CompletedDate) {
      axios
        .post(
          `http://localhost:5566/todo/ByDate/${Status}/${StartDate.toISOString()}/${CompletedDate.toISOString()}`
        )
        .then((response) => {
  ...
  const handleSave = async () => {
    ...
    const fetchPage = async (page: number) => {
      const response = await axios.post("http://localhost:5566/todo/readAll", {
        page,
      });
